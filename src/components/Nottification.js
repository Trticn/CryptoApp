...
import { hideNottification } from "../store"; // prilagodi putanju ako treba
...
function Notification({ onClose }) {
  ...
  const colorClasses = {
    ...
    warning:
      "bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-300 border-yellow-200 dark:border-yellow-700",
  }[type] || "bg-gray-800 text-white";

  return (
    <div
      className={`fixed top-16 right-4 z-50 min-w-[220px] max-w-xs px-4 py-3 rounded-lg shadow-lg flex items-center gap-2 ${colorClasses} animate-slide-in`}
    ...
    >
      ...
      <button
        ...
        className="ml-2 text-white/80 hover:text-white font-bold text-lg focus:outline-none"
